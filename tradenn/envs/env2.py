...
class State:
    ...
    def maybe_liquidate(self, transaction_fee: float = 0.01, slippage: float = 0.01):
        total_liq = 0.0
        did_something = True
        while self.margin_shortfall > 0 and did_something:
            did_something = False
            for asset in reversed(np.argsort(self.array[3, :])):
                if self.array[3, asset] == 0:
                    continue
                if self.array[3, asset] > 0:
                    amount_to_sell = min(
                        self.array[3, asset],
                        math.ceil(
                            self.margin_shortfall
                            / (
                                self.array[1, asset]
                                * (1 - slippage)
                                * (1 - transaction_fee)
                    ...
                    if amount_to_sell > 0:
                        # print(
                        #     f"Sell {self.array[3, asset]} {asset} {amount_to_sell} {self.array[1, asset]}"
                        # )
                        liq = self.sell(
                            np.array([asset]),
                    ...
                    if max_cover > 0:
                        # print(
                        #     f"Buy {self.array[3, asset]} {asset} {max_cover} {self.array[2, asset]}"
                        # )
                        liq = self.buy(
                            np.array([asset]),
                            np.array([max_cover]),
    ...
    def observation(self):
        ...
        array[-1, :] = self.exposure * MAINT_MARGIN / self.equity

        return array

    @property
    def equity(self):
        return self.cash + np.sum(self.array[3, :] * self.array[2, :])
    ...
    def margin_shortfall(self):
        return self.exposure * MAINT_MARGIN - self.equity
...
class StockEnv(gym.Env):
    ...
    def step(self, actions, seed=0):
        ...
        if not self.terminal:
            ...
            # Check for insolvency first
            if self.state.net_liq_value <= 0:
                self.terminal = True
                self.reward = -2.0  # Severe penalty for insolvency
                end_total_asset = 0.0
                print("Terminated: Net liquidation value <= 0")
            else:
                # Attempt liquidation if needed
                total_liq = self.state.maybe_liquidate(
                    self.transaction_fee, self.slippage
                )
                if total_liq > 0:
                    self.num_liquidations += 1
                    liq_penalty = total_liq / self.initial_balance * 5
                    penalty += liq_penalty
                    print(f"Liquidation penalty {liq_penalty} for {total_liq}")

                if self.state.cash < self.initial_balance * 0.02:
                    cash_penalty = (
                        1 - self.state.cash / (self.initial_balance * 0.02)
                    ) ** 2
            ...
            self.asset_memory.append(max(0, end_total_asset))
